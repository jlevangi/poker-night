#!/usr/bin/env python3
"""
Database migration script to add is_cashed_out field to entries table.

This migration adds the new boolean field 'is_cashed_out' to the entries table
to track whether a player is currently cashed out in a session.
"""
...
def migrate_database(database_path):
    """
    Add is_cashed_out column to entries table.
    ...
    try:
        ...
        if 'is_cashed_out' in columns:
            print(f"Column 'is_cashed_out' already exists in {database_path}")
            return
        
        print(f"Adding 'is_cashed_out' column to entries table in {database_path}...")
        ...
        print(f"Migration completed successfully! Updated {affected_rows} existing entries.")
...
def main():
    ...
    print("\nIMPORTANT: The cashout button behavior has been updated to use a boolean flag.")
    print("Players with existing payouts have been automatically marked as 'cashed out'.")
